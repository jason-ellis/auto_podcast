...
class Rss(object):
    ...
    def create_xml(self):
        ...
        #rss_xml.set('xmlns:itunes', 'http://www.itunes.com/dtds/podcast-1.0.dtd')
        ...
        output_file = open(constants.RSS_DIR + constants.RSS_FILENAME, 'w')
    ...
    def add_rss(self, podcast):
        ...
        if not isfile(constants.RSS_DIR + constants.RSS_FILENAME):
            self.create_xml()
        else:
            xml_tree = parse(constants.RSS_DIR + constants.RSS_FILENAME)
            ...
            if podcast['title'] not in xml_titles:
                ...
                xml_tree.write(constants.RSS_DIR + constants.RSS_FILENAME)
            else:
                if constants.VERBOSE:
